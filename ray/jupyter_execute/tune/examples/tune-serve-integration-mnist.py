...
# is the ``from ray import train, tune, serve`` import statement - which
...
def train_mnist(
    ...
    elif train.get_checkpoint():  # load from previous checkpoint
        with train.get_checkpoint().as_directory() as loaded_checkpoint_dir:
    ...
    for i in range(num_epochs):
        ...
        acc = test_func(model, validation_loader, device)
        if i == num_epochs - 1:
            ...
            train.report({"mean_accuracy": acc, "done": True}, checkpoint=checkpoint)
        else:
...
def tune_from_scratch(num_samples=10, num_epochs=10, gpus_per_trial=0.0, day=0):
    ...
    best_accuracy = best_result.metrics["mean_accuracy"]
...
def tune_from_existing(
    ...
    tuner = tune.Tuner(
        ...
        run_config=train.RunConfig(
            name="tune_serve_mnist_fromexisting",
            progress_reporter=reporter,
            verbose=0,
        ),
        param_space=config,
